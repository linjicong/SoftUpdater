...
├── history.json             # 更新历史（最近 200 条：时间/软件/版本变化/结果，自动生成）
├── logs\                    # 按天滚动日志（UI 与定时任务共用）
└── tests\run-tests.ps1      # 核心逻辑测试套件（166 断言，无 Pester 依赖）
```
...
| 只看有更新 | 勾选「只看有更新」 |
| 搜索过滤 | 顶栏输入关键字按名称/ID 实时过滤（与分类筛选叠加），✕ 清除 |
...
- 匹配规则：exe 路径落在该软件安装目录下 → 直接命中；快捷方式条目/裸别名（如 `AlibabaCloud.Qoder`）按名称宽松匹配（跳过 setup/uninstall 等通用安装器名，防误匹配）；注册表 DisplayIcon 提取的主程序 exe 名（ExeHint）第三重兜底；一行取所有命中记录的最新时间；
...
- 日志区实时显示 winget 输出；磁盘日志在 `logs\yyyy-MM-dd.log`，**超 30 天自动清理**；
- 每次成功升级自动记入 `history.json`（保留最近 200 条：时间、软件、原版本 → 新版本、结果）；
- 更新结束**不做全量重扫**：只对刚更新的软件按注册表（版本权威来源）秒级校正版本与状态，其余软件的新版本变化由下次「刷新列表」/定时任务发现。
...
- 典型耗时：脚本自身启动 ~1s；winget 枚举 ~15-20s（后台线程，不阻塞界面，偶发源刷新时更久）；注册表 ~0.5s；目录扫描 ~0.3s；合并 ~3.5s；最近使用 ~0.6s（UserAssist 读取 ~0.3s + 行匹配 ~0.3s，倒排索引加速）；
- 每次枚举结束自动输出**分段耗时日志**（winget/注册表/目录/合并/最近使用），卡顿打开当天日志即可定位慢在哪一步；
...
- **GitHub 源**：条目可带 `"GithubRepo": "owner/repo"`（检测时也会从 github.com 安装包地址自动推导并回写），检测改走 GitHub Releases 最新版——tag 去 v 前缀比版本、自动挑选 x64 的 .exe/.msi 资产、发布页写入 DownloadPage；未认证 API 限流 60 次/时，默认 24h TTL 完全够用；
